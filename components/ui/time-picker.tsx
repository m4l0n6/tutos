    ...
    <TimePickerInputGroupContext.Provider value={inputGroupContextValue}>
      <PopoverAnchor asChild>
        <InputGroupPrimitive
          ...
          {...inputGroupProps}
          className={cn(
            "flex items-center gap-0.5 bg-background shadow-xs px-3 py-2 border border-input rounded-md outline-none w-full h-10 transition-shadow cursor-text",
...
function TimePickerInput(props: TimePickerInputProps) {
  ...
  return (
    <input
      ...
      readOnly={isReadOnly}
      className={cn(
        "inline-flex justify-center items-center bg-transparent focus:bg-transparent disabled:opacity-50 border-0 outline-none h-full tabular-nums text-sm text-center transition-colors disabled:cursor-not-allowed",
        className,
      )}
...
function TimePickerColumnItem(props: TimePickerColumnItemProps) {
  ...
  return (
    <button
      ...
      className={cn(
        "hover:bg-accent px-3 py-1.5 focus:border-ring rounded focus:outline-none focus:ring-[3px] focus:ring-ring/50 w-full text-sm text-left hover:text-accent-foreground",
...
function TimePickerHour(props: TimePickerHourProps) {
  ...
  return (
    <HourPrimitive
      ...
      {...hourProps}
      className={cn(
        "flex flex-col gap-1 p-1 max-h-[200px] overflow-y-auto scrollbar-none",
        className,
      )}
    >
      {hours.map((hour) => (
...
function TimePickerClear(props: ButtonProps) {
  ...
  return (
    <ClearPrimitive
      ...
      {...clearProps}
      className={cn(
        "inline-flex justify-center items-center hover:bg-accent disabled:opacity-50 rounded-sm font-medium text-sm transition-colors hover:text-accent-foreground disabled:pointer-events-none",
        className,
      )}
      onClick={onClick}
    >
