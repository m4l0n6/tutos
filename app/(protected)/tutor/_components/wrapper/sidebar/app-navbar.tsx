...
function checkBreadcrumb(pathname: string): string[] {
  const segments = pathname
    .split("/")
    .filter((segment) => segment && segment !== "tutor")

  return ["Tutor", ...segments.map((seg) => seg.charAt(0).toUpperCase() + seg.slice(1))]
}

export function AppNavbar() {
  const pathname = usePathname()
  const [breadcrumbs, setBreadcrumbs] = useState<string[]>(["Tutor"])
